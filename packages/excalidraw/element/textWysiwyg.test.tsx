...
import {
  GlobalTestState, fireEvent,
  mockBoundingClientRect, render, restoreOriginalGetBoundingClientRect, screen
...
describe("textWysiwyg", () => {
  ...
  describe("Test container-bound text", () => {
    ...
    it("should update line height when font family updated", async () => {
      ...
      expect(
        (h.elements[1] as ExcalidrawTextElementWithContainer).fontFamily,
      ).toEqual(FONT_FAMILY["libre bodoni"]);
      expect(
        (h.elements[1] as ExcalidrawTextElementWithContainer).lineHeight,
      ).toEqual(1.15);
    });
